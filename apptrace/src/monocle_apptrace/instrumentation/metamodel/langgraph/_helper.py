...
def _as_state_dict(state):
    """Normalize a StateGraph state to a dict for channel introspection.

    LangGraph states are TypedDicts (plain dicts at runtime) by convention, but custom
    StateGraphs also commonly use a dataclass or Pydantic BaseModel schema. Convert those
    shallowly (no recursive serialization) so nested message objects keep their attributes.
    """
